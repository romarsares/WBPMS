...
2. WHEN a new employee account is provisioned THEN the system SHALL use the
   employee's email address as the login credential. HR SHALL supply the
   employee's email address during employee creation. No username field
   exists; email is the only login identifier. [REQ010 extension; HR
   interview 2026-09-21]
3. IF no email address is supplied for the new employee THEN the system SHALL
   block account provisioning and require HR to provide a valid email address
   before the employee record can be saved.
...
7. WHEN a new employee account is provisioned THEN the system SHALL display
   the assigned email address and the temporary password to the HR Head
   exactly once, in a dismissible confirmation dialog, so HR can relay
   credentials to the employee securely.
